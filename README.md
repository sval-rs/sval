...
3. **Buffer-friendly.** The [`Stream`](https://docs.rs/sval/2.18.0/sval/trait.Stream.html) trait is non-recursive, so values can be buffered as a flat stream of tokens and replayed later.
...
5. **Broadly compatible.** `sval` imposes very few constraints of its own, so it can trivially translate implementations of [`serde::Serialize`](https://docs.rs/serde/latest/serde/trait.Serialize.html) into implementations of [`sval::Value`](https://docs.rs/sval/2.18.0/sval/trait.Value.html).
...
First, add [`sval_json`](https://docs.rs/sval_json/2.18.0/sval_json/index.html) to your `Cargo.toml`:
...
Next, derive the [`Value`](https://docs.rs/sval/2.18.0/sval/trait.Value.html) trait on the type you want to serialize, including on any other types it uses in its fields:
...
Finally, use [`stream_to_string`](https://docs.rs/sval_json/2.18.0/sval_json/fn.stream_to_string.html) to serialize an instance of your type as JSON:
...
If we wanted to serialize `Template` to a string, we could implement [`Value`](https://docs.rs/sval/2.18.0/sval/trait.Value.html), handling each literal and property as a separate fragment:
...
Implementations of [`Value`](https://docs.rs/sval/2.18.0/sval/trait.Value.html) should provide a [`Stream`](https://docs.rs/sval/2.18.0/sval/trait.Stream.html) with borrowed data where possible, and only compute it if it needs to.

### Error handling

`sval`'s [`Error`](https://docs.rs/sval/2.18.0/sval/struct.Error.html) type doesn't carry any state of its own. It only signals early termination of the [`Stream`](https://docs.rs/sval/2.18.0/sval/trait.Stream.html) which may be because its job is done, or because it failed. It's up to the [`Stream`](https://docs.rs/sval/2.18.0/sval/trait.Stream.html) to carry whatever state it needs to provide meaningful errors.
...
The [`Label`](https://docs.rs/sval/2.18.0/sval/struct.Label.html) and [`Index`](https://docs.rs/sval/2.18.0/sval/struct.Index.html) types can also carry a [`Tag`](https://docs.rs/sval/2.18.0/sval/struct.Tag.html). An example of a [`Tag`](https://docs.rs/sval/2.18.0/sval/struct.Tag.html) you might use on a [`Label`](https://docs.rs/sval/2.18.0/sval/struct.Label.html) is [`VALUE_IDENT`](https://docs.rs/sval/2.18.0/sval/tags/constant.VALUE_IDENT.html), for labels that hold a valid Rust identifier.

### Type system

`sval` has an implicit structural type system based on the sequence of calls a [`Stream`](https://docs.rs/sval/2.18.0/sval/trait.Stream.html) receives, and the values of any [`Label`](https://docs.rs/sval/2.18.0/sval/struct.Label.html), [`Index`](https://docs.rs/sval/2.18.0/sval/struct.Index.html), or [`Tag`](https://docs.rs/sval/2.18.0/sval/struct.Tag.html) on them, with the following exceptions:
