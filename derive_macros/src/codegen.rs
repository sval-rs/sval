...
use crate::attr::RefLifetimeSpec;
...
fn field_codegen(attrs: &[Attribute]) -> syn::Result<Option<FieldCodegen>> {
    let outer_ref =
        crate::attr::get("struct field", crate::attr::OuterRefAttr, attrs)?.unwrap_or(false);
    let inner_ref =
        crate::attr::get("struct field", crate::attr::InnerRefAttr, attrs)?.unwrap_or(false);
    let computed =
        crate::attr::get("struct field", crate::attr::ComputedAttr, attrs)?.unwrap_or(false);
...
pub(crate) fn stream_value_tokens(
    binding: &Ident,
    attrs: &[Attribute],
...
pub(crate) trait ImplStrategy {
    ...
    fn generate(&self, ident: &Ident, generics: &Generics, stream_body: TokenStream)
        -> TokenStream;
...
impl ImplStrategy for ValueImpl {
    fn default_field_codegen(&self) -> FieldCodegen {
...
pub(crate) struct ValueRefImpl {
    pub(crate) lifetime_spec: RefLifetimeSpec,
    pub(crate) inner_ref_field_types: Vec<syn::Type>,
}

impl ValueRefImpl {
    pub(crate) fn new(
        lifetime_spec: RefLifetimeSpec,
        inner_ref_field_types: Vec<syn::Type>,
    ) -> Self {
        Self {
            lifetime_spec,
            inner_ref_field_types,
        }
    }
}

impl ImplStrategy for ValueRefImpl {
    fn default_field_codegen(&self) -> FieldCodegen {
    ...
    fn generate(
        ...
        let lifetime = &self.lifetime_spec.lifetime;
        let bounds = &self.lifetime_spec.bounds;
        ...
        for field_type in &self.inner_ref_field_types {
...
pub(crate) fn infer_ref_lifetime(generics: &Generics) -> syn::Result<RefLifetimeSpec> {
    let lifetimes: Vec<_> = generics.lifetimes().map(|lt| lt.lifetime.clone()).collect();

    match lifetimes.len() {
        1 => Ok(RefLifetimeSpec {
